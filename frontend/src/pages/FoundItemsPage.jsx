...
export default function FoundItemsPage() {
    ...
    const loadItems = () => {
        api.get("/found")
            ...
            .catch((err) => {
                ...
                setMessage({ type: "error", text: "Unable to load found items." });
            });
    };
    ...
    const markReturned = async (id) => {
        try {
            await api.patch(`/found/${id}/return`);
            setMessage({ type: "success", text: "Found item marked as returned." });
            loadItems();
        } catch (err) {
            console.error("Error marking found item returned:", err);
                ...
                <table className="data-table">
                    ...
                    <tbody>
                        ...
                        {filteredItems.length === 0 && (
                            <tr>
                                <td colSpan="7" className="empty-state">No found items match these filters.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
